...
def copie_urls_livre(url_a_parcourir):
    ...
    # parcours l'url de la categorie et copie tous les liens pointant
    # vers une page "livre" dans la liste "liste_urls_livres"
    for i in url_boucle:
        lien = i.find("a")
        lien = lien["href"]
...
def scrap_page_livre(url_page_livre):
    ...
    if req.ok:
        ...
        # utilise urllib pour télécharger l'image dans le dossier 'image_scrap",
        # le nom du fichier est l'upc suivi du titre du livre
        urllib.request.urlretrieve(image_source,
                                   "images_scrap/" + upc + " - " + titre.replace('"', '')
